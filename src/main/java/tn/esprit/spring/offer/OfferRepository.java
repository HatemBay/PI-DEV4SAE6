...
public interface OfferRepository extends JpaRepository<Offer, Integer>, CrudRepository<Offer, Integer>{
	...
	@Query(value= "Select * FROM Offer where user_id = :id", 
			nativeQuery=  true)
	public List<Offer> getOfferPerUser(@Param("id") int id);
	
	@Query(value= "Select * FROM Offer where seller_id = :id", 
			nativeQuery=  true)
	public List<Offer> getOfferPerSeller(@Param("id") int id);
